...
The type system of Phi is relatively straightforward, except for the handling of indices. Type constructors are unconstrained, and even allowing arrays of pairs (missing from common Python array libraries).
\begin{align*}
...
Since reductions are a special case of a fold (\mintinline{python}{v.reduce(id, f)} $\leadsto$ \mintinline{python}{fold(id, lambda i, x: f(x, v[i]))}), we do not elaborate on them further. All relevant techniques (AoS-to-SoA, Axials, etc.) can be adapted.
